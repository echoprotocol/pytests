...
from project import BLOCKS_NUM_TO_WAIT, GENESIS
...
class Utils(object):
    ...
    @staticmethod
    def add_balance_for_operations(
        base_test,
        account,
        operation,
        database_api_id,
        ...
        fee_amount = base_test.get_required_fee(operation, database_api_id, fee_asset_id)["amount"]
        if type(fee_amount) is str:
            fee_amount = int(fee_amount)
        amount = operation_count * fee_amount + transfer_amount + deposit_amount
        transfer_amount_for_pay_fee_op = base_test.echo_ops.get_transfer_operation(
            echo=base_test.echo, from_account_id=base_test.echo_acc0, to_account_id=account, amount=amount
        )
        collected_operation = base_test.collect_operations(transfer_amount_for_pay_fee_op, database_api_id)
        return base_test.echo_ops.broadcast(
            echo=base_test.echo, list_operations=collected_operation, log_broadcast=log_broadcast
        )
    ...
    def get_nonexistent_asset_symbol(self, base_test, database_api_id, symbol="", list_asset_symbols=None):
        ...
        if len(response["result"]) == max_limit:
            return self.get_nonexistent_asset_symbol(
                base_test, database_api_id, symbol=list_asset_symbols[-1], list_asset_symbols=list_asset_symbols
            )
    ...
    def get_nonexistent_account_name(
        self, base_test, database_api_id, lower_bound_name="", list_account_names=None, committee_member=False
    ):
        if list_account_names is None:
            list_account_names = []
        method_name = "lookup_accounts" if not committee_member else "lookup_committee_member_accounts"
        max_limit = 1000
        response_id = base_test.send_request(
            base_test.get_request(method_name, [lower_bound_name, max_limit]), database_api_id
        )
        response = base_test.get_response(response_id)
        for account in response["result"]:
            list_account_names.append(account[0])
        if len(response["result"]) == max_limit:
            return self.get_nonexistent_account_name(
                base_test,
                database_api_id,
    ...
    def get_nonexistent_committee_member_id(base_test, database_api_id):
        ...
        return "{}{}".format(
            base_test.get_object_type(base_test.echo.config.object_types.COMMITTEE_MEMBER), committee_count
        )
    ...
    def get_contract_id(
        ...
        response_id = base_test.send_request(
            base_test.get_request("get_contract_result", [contract_result]), database_api_id
        )
        ...
        return {
            "contract_id": base_test.get_contract_id(contract_id),
            "broadcast_result": broadcast_result
        }

    def perform_contract_call_operation(
        self,
        base_test,
        registrar,
        method_bytecode,
        ...
        broadcast_result = base_test.echo_ops.broadcast(
            echo=base_test.echo, list_operations=list_operations, log_broadcast=log_broadcast
        )
        return broadcast_result
    ...
    def perform_transfer_to_address_operations(
        self,
        base_test,
        account_1,
        to_address,
    ...
    def check_accounts_have_initial_balances(accounts):
        initial_accounts = {
            account["name"]: account["echorand_key"]
        ...
        initial_balances_keys = list(
            filter(lambda balance: balance in initial_accounts.values(), initial_balances_keys)
        )
    ...
    def get_asset_id(base_test, symbol, database_api_id, need_operation=False, log_broadcast=False):
        params = [symbol, 1]
        response_id = base_test.send_request(
            base_test.get_request("list_assets", params),
            database_api_id,
        )
        response = base_test.get_response(response_id)
        if not response["result"] or response["result"][0]["symbol"] != symbol:
            operation = base_test.echo_ops.get_asset_create_operation(
                echo=base_test.echo, issuer=base_test.echo_acc0, symbol=symbol
            )
            collected_operation = base_test.collect_operations(operation, database_api_id)
            broadcast_result = base_test.echo_ops.broadcast(
                echo=base_test.echo, list_operations=collected_operation, log_broadcast=log_broadcast
    ...
    def add_assets_to_account(base_test, value, asset_id, to_account, database_api_id, log_broadcast=False):
        operation = base_test.echo_ops.get_asset_issue_operation(
            echo=base_test.echo,
            issuer=base_test.echo_acc0,
            value_amount=value,
    ...
    def perform_account_address_create_operation(
        self, base_test, registrar, label, database_api_id, operation_count=1, log_broadcast=False
    ):
        operation = base_test.echo_ops.get_account_address_create_operation(
            echo=base_test.echo, owner=registrar, label=label
        )
        if registrar != base_test.echo_acc0:
            temp_operation = deepcopy(operation)
            broadcast_result = self.add_balance_for_operations(
        ...
        if operation_count == 1:
            ...
            if not base_test.is_operation_completed(broadcast_result, expected_static_variant=1):
                raise Exception(
                    "Error: new address of '{}' account is not created, "
        ...
        if not base_test.is_operation_completed(broadcast_result, expected_static_variant=0):
            raise Exception(
                "Error: new eth address of '{}' account is not created, response:\n{}".format(
                    registrar, broadcast_result
                )
            )
        return broadcast_result

    @staticmethod
    def perform_sidechain_eth_withdraw_operation(
        base_test, registrar, eth_addr, value, database_api_id, log_broadcast=False
    ):
        if eth_addr[:2] == "0x":
            eth_addr = eth_addr[2:]
        operation = base_test.echo_ops.get_sidechain_eth_withdraw_operation(
            echo=base_test.echo, account=registrar, eth_addr=eth_addr, value=value
        )
        if registrar != base_test.echo_acc0:
            temp_operation = deepcopy(operation)
            broadcast_result = base_test.utils.add_balance_for_operations(base_test, registrar, temp_operation,
    ...
    def set_timeout_until_num_blocks_released(self, base_test, database_api_id, wait_block_count=1, print_log=True):
        ...
        response_id = base_test.send_request(
            base_test.get_request("set_block_applied_callback", [callback]), database_api_id
        )
        response = base_test.get_response(response_id)
        if "result" not in response or response["result"] is not None:
        ...
        if print_log:
            lcc.log_info(
                "Waited for release of '{}' block(s). Wait time: '{}' seconds"
    ...
    def get_eth_address(self, base_test, account_id, database_api_id, wait_time=0, temp_count=0):
        ...
        raise Exception("No ethereum address of '{}' account. Waiting time result='{}'".format(account_id, wait_time))
    ...
    def get_eth_balance(self, base_test, account_id, database_api_id, previous_balance=None, wait_time=0, temp_count=0):
        current_balance = self.get_account_balances(base_test, account_id, database_api_id,
                                                    base_test.eth_asset)["amount"]
    ...
    def get_updated_address_balance_in_eth_network(
        self,
        base_test,
        account_address,
        previous_balance,
        ...
        current_balance = base_test.eth_trx.get_address_balance_in_eth_network(
            base_test.web3, account_address, currency=currency
        )
        ...
        if temp_count <= BLOCKS_NUM_TO_WAIT:
            wait_time += self.set_timeout_until_num_blocks_released(base_test, database_api_id, print_log=False)
            return self.get_updated_address_balance_in_eth_network(
                base_test,
                account_address,
                previous_balance,
                database_api_id,
    ...
    def perform_vesting_balance_create_operation(
        self,
        base_test,
        creator,
        owner,
        ...
        operation = base_test.echo_ops.get_vesting_balance_create_operation(
            echo=base_test.echo,
            creator=creator,
            owner=owner,
            amount=amount,
        ...
        if creator != base_test.echo_acc0:
            temp_operation = deepcopy(operation)
            broadcast_result = self.add_balance_for_operations(
                base_test, creator, temp_operation, database_api_id, log_broadcast=log_broadcast
            )
            if not base_test.is_operation_completed(broadcast_result, expected_static_variant=0):
                raise Exception("Error: can't add balance to new account, response:\n{}".format(broadcast_result))
        collected_operation = base_test.collect_operations(operation, database_api_id)
        ...
        if not base_test.is_operation_completed(broadcast_result, expected_static_variant=1):
            raise Exception(
                "Error: vesting balance of '{}' account is not withdrawn, response:\n{}".format(
                    owner, broadcast_result
                )
            )
        return broadcast_result

    def perform_vesting_balance_withdraw_operation(
        self, base_test, vesting_balance, owner, amount, database_api_id, amount_asset_id="1.3.0", log_broadcast=False
    ):
        operation = base_test.echo_ops.get_vesting_balance_withdraw_operation(
            echo=base_test.echo,
        ...
        if owner != base_test.echo_acc0:
            temp_operation = deepcopy(operation)
            broadcast_result = self.add_balance_for_operations(
                base_test, owner, temp_operation, database_api_id, log_broadcast=log_broadcast
            )
            if not base_test.is_operation_completed(broadcast_result, expected_static_variant=0):
                raise Exception("Error: can't add balance to new account, response:\n{}".format(broadcast_result))
        collected_operation = base_test.collect_operations(operation, database_api_id)
    ...
    def get_account_history_operations(
        self,
        base_test,
        account_id,
        operation_id,
        ...
        response_id = base_test.send_request(
            base_test.get_request("get_account_history_operations", params), history_api_id
        )
        response = base_test.get_response(response_id)
        if "error" in response:
            if temp_count <= BLOCKS_NUM_TO_WAIT:
                wait_time += self.set_timeout_until_num_blocks_released(base_test, database_api_id, print_log=False)
                return self.get_account_history_operations(
                    base_test,
                    account_id,
                    operation_id,
                    history_api_id,
    ...
    def perform_contract_fund_pool_operation(
        ...
        if sender != base_test.echo_acc0:
            temp_operation = deepcopy(operation)
            broadcast_result = self.add_balance_for_operations(
                base_test, sender, temp_operation, database_api_id, log_broadcast=log_broadcast
            )
            if not base_test.is_operation_completed(broadcast_result, expected_static_variant=0):
                raise Exception("Error: can't add balance to new account, response:\n{}".format(broadcast_result))
        collected_operation = base_test.collect_operations(operation, database_api_id)
    ...
    def perform_contract_whitelist_operation(
        self,
        base_test,
        sender,
        contract,
        ...
        operation = base_test.echo_ops.get_contract_whitelist_operation(
            echo=base_test.echo,
            sender=sender,
            contract=contract,
    ...
    def perform_committee_member_create_operation(
        self,
        base_test,
        account_id,
        eth_address,
        ...
        operation = base_test.echo_ops.get_committee_member_create_operation(
            echo=base_test.echo,
        ...
        if account_id != base_test.echo_acc0:
            temp_operation = deepcopy(operation)
            broadcast_result = self.add_balance_for_operations(
                base_test, account_id, temp_operation, database_api_id, deposit_amount, log_broadcast=log_broadcast
            )
            if not base_test.is_operation_completed(broadcast_result, expected_static_variant=0):
                raise Exception("Error: can't add balance to new account, response:\n{}".format(broadcast_result))
        collected_operation = base_test.collect_operations(operation, database_api_id)
        ...
        if not base_test.is_operation_completed(broadcast_result, expected_static_variant=1):
            raise Exception(
                "Error: '{}' account did not become new committee member, response:\n{}".format(
                    account_id, broadcast_result
                )
            )
        return broadcast_result

    def perform_committee_member_update_operation(
        self,
        base_test,
        committee_member,
        ...
        if committee_member_account != base_test.echo_acc0:
            temp_operation = deepcopy(operation)
            broadcast_result = self.add_balance_for_operations(
                base_test, committee_member_account, temp_operation, database_api_id, log_broadcast=log_broadcast
            )
            if not base_test.is_operation_completed(broadcast_result, expected_static_variant=0):
                raise Exception("Error: can't add balance to new account, response:\n{}".format(broadcast_result))
        collected_operation = base_test.collect_operations(operation, database_api_id)
        ...
        if not base_test.is_operation_completed(broadcast_result, expected_static_variant=0):
            raise Exception(
                "Error: '{}' committee member did not updated, response:\n{}"
                "".format(committee_member_account, broadcast_result)
            )
        return broadcast_result

    def perform_committee_member_activate_operation(
        self, base_test, committee_member, committee_member_account, database_api_id, signer=None, log_broadcast=False
    ):
        operation = \
        ...
        if not base_test.is_operation_completed(broadcast_result, expected_static_variant=0):
            raise Exception(
                "Error: '{}' committee member did not activate, response:\n{}"
                "".format(committee_member, broadcast_result)
            )
        return broadcast_result

    def perform_account_update_operation(
        self, base_test, account_id, account_info, database_api_id, log_broadcast=False
    ):
        active, echorand_key, options = account_info["active"], account_info["echorand_key"], account_info["options"]
        operation = base_test.echo_ops.get_account_update_operation(
            echo=base_test.echo,
            account=account_id,
        ...
        if account_id != base_test.echo_acc0:
            temp_operation = deepcopy(operation)
            broadcast_result = self.add_balance_for_operations(
                base_test, account_id, temp_operation, database_api_id, log_broadcast=log_broadcast
            )
            if not base_test.is_operation_completed(broadcast_result, expected_static_variant=0):
                raise Exception("Error: can't add balance to new account, response:\n{}".format(broadcast_result))
        collected_operation = base_test.collect_operations(operation, database_api_id)
        ...
        if not base_test.is_operation_completed(broadcast_result, expected_static_variant=0):
            raise Exception("Error: '{}' account did not update, response:\n{}".format(account_id, broadcast_result))
        return broadcast_result

    def perform_contract_update_operation(
        self, base_test, sender, contract, database_api_id, new_owner=None, log_broadcast=False
    ):
        operation = base_test.echo_ops.get_contract_update_operation(
            echo=base_test.echo, sender=sender, contract=contract, new_owner=new_owner
        )
        if sender != base_test.echo_acc0:
            temp_operation = deepcopy(operation)
            broadcast_result = self.add_balance_for_operations(
    ...
    def get_erc20_account_deposits(
        self, base_test, account_id, database_api_id, wait_time=0, previous_account_deposits=None, temp_count=0
    ):
        response_id = base_test.send_request(
            base_test.get_request("get_erc20_account_deposits", [account_id]), database_api_id
        )
        response = base_test.get_response(response_id)
        ...
        if temp_count <= BLOCKS_NUM_TO_WAIT:
            ...
            return self.get_erc20_account_deposits(
                base_test,
                account_id,
                database_api_id,
                wait_time,
        ...
        raise Exception("No needed '{}' account erc20 deposits. Waiting time result='{}'".format(account_id, wait_time))

    def get_erc20_account_withdrawals(
        self, base_test, account_id, database_api_id, wait_time=0, previous_account_withdrawals=None, temp_count=0
    ):
        response_id = base_test.send_request(
            base_test.get_request("get_erc20_account_withdrawals", [account_id]), database_api_id
        )
        response = base_test.get_response(response_id)
        ...
        if temp_count <= BLOCKS_NUM_TO_WAIT:
            base_test.produce_block(database_api_id)
            return self.get_erc20_account_withdrawals(
                base_test,
                account_id,
                database_api_id,
                wait_time,
    ...
    def get_updated_account_erc20_balance_in_eth_network(
        self, base_test, contract_instance, eth_account, previous_balance, database_api_id, wait_time=0, temp_count=0
    ):
        ...
        if temp_count <= BLOCKS_NUM_TO_WAIT:
            wait_time += self.set_timeout_until_num_blocks_released(base_test, database_api_id, print_log=False)
            return self.get_updated_account_erc20_balance_in_eth_network(
                base_test,
                contract_instance,
                eth_account,
                previous_balance,
    ...
    def get_erc20_token_balance_in_echo(
        self,
        base_test,
        account_id,
        balance_of_method,
        ...
        if temp_count <= BLOCKS_NUM_TO_WAIT:
            base_test.produce_block(database_api_id)
            return self.get_erc20_token_balance_in_echo(
                base_test,
                account_id,
                balance_of_method,
                contract_id,
    ...
    def get_account_block_reward(self, base_test, account_ids, block_number, database_api_id):
        total_verifiers_balance, total_verifiers_reward, delegate_reward, account_reward = 0, 0, 0, 0
        ...
        response_id = base_test.send_request(base_test.get_request("get_block", [block_number]), database_api_id)
        ...
        if verifier:
            ...
            for param in params:
                response_id = base_test.send_request(
                    base_test.get_request("get_account_balances", param), database_api_id
                )
