...
class ObjectValidator(object):
    ...
    def validate_account_object(self, base_test, account_info):
        if check_that("account info", account_info, has_length(16), quiet=True):
            check_that_in(
                account_info,
                "active",
                is_dict(),
            ...
            if check_that("active", account_info["active"], has_length(3), quiet=True):
                check_that_in(
                    account_info["active"],
                    "weight_threshold",
                    is_integer(),
            ...
            if check_that("options", account_info["options"], has_length(3), quiet=True):
                ...
                check_that_in(
                    account_info["options"], "delegate_share", is_integer(), "extensions", is_list(), quiet=True
                )

    def validate_asset_object(self, base_test, asset):

        def validate_core_exchange_rate_structure(base_test, core_exchange_rate):
            check_that_in(core_exchange_rate, "base", is_dict(), "quote", is_dict(), quiet=True)
            for key in core_exchange_rate:
                ...
                if not base_test.type_validator.is_asset_id(core_exchange_rate_part["asset_id"]):
                    lcc.log_error(
                        "Wrong format of {} 'asset_id', got: {}".format(key, core_exchange_rate_part["asset_id"])
                    )
                else:
                    lcc.log_info("{} 'asset_id' has correct format: asset_id".format(key))

        if check_that("'length of chain asset'", asset, has_length(7), quiet=True):
            ...
            if not base_test.type_validator.is_dynamic_asset_data_id(asset["dynamic_asset_data_id"]):
                lcc.log_error("Wrong format of 'dynamic_asset_data_id', got: {}".format(asset["dynamic_asset_data_id"]))
            else:
            ...
            check_that_in(asset, "options", is_dict(), "extensions", is_list(), "precision", is_integer(8), quiet=True)
            options = asset["options"]
            if check_that("'options'", options, has_length(8), quiet=True):
                check_that_in(
                    options,
                ...
                if check_that("'core_exchange_rate'", core_exchange_rate, has_length(2), quiet=True):
    ...
    def validate_committee_member_object(self, base_test, committee_member):
        if check_that("'committee member'", committee_member, has_length(6), quiet=True):
            ...
            if not base_test.type_validator.is_account_id(committee_member["committee_member_account"]):
                lcc.log_error(
                    "Wrong format of 'committee_member_account', got: {}".format(
            ...
            if not base_test.type_validator.is_eth_address(committee_member["eth_address"]):
                lcc.log_error("Wrong format of 'eth_address', got: {}".format(committee_member["eth_address"]))
            else:
            ...
            if not base_test.type_validator.is_btc_public_key(committee_member["btc_public_key"]):
                lcc.log_error("Wrong format of 'btc_public_key', got: {}".format(committee_member["btc_public_key"]))
            else:
                lcc.log_info("'eth_address' has correct format: hex")
            check_that_in(committee_member, "url", is_str(), "extensions", is_list(), quiet=True)

    def validate_proposal_object(self, base_test, proposal_object):
        if check_that("'proposal'", proposal_object, has_length(7), quiet=True):
            ...
            if check_that("'proposed transaction'", proposed_transaction, has_length(5), quiet=True):
    ...
    def validate_operation_history_object(self, base_test, operation_history_object):
        if check_that("operation history", operation_history_object, has_length(8), quiet=True):
    ...
    def validate_vesting_balance_object(self, base_test, vesting_balance_object):
        if check_that("vesting balance", vesting_balance_object, has_length(5), quiet=True):
            ...
            if check_that("balance", balance, has_length(2), quiet=True):
            ...
            if check_that("policy", policy, has_length(2), quiet=True):
    ...
    def validate_balance_object(self, base_test, balance_object):
        if check_that("balance", balance_object, has_length(5), quiet=True):
            ...
            if not base_test.type_validator.is_iso8601(balance_object["last_claim_date"]):
                lcc.log_error("Wrong format of 'last_claim_date', got: {}".format(balance_object["last_claim_date"]))
            else:
            ...
            if not base_test.type_validator.is_asset_id(balance_object["balance"]["asset_id"]):
                lcc.log_error("Wrong format of 'asset_id', got: {}".format(balance_object["balance"]["asset_id"]))
            else:
                lcc.log_info("'asset_id' has correct format: asset_object_type")
            check_that_in(balance_object, "extensions", is_list(), quiet=True)

    def validate_frozen_balance_object(self, base_test, frozen_balance_object):
        if check_that("frozen balance", frozen_balance_object, has_length(6), quiet=True):
            check_that_in(
            ...
            if check_that("balance", balance, has_length(2), quiet=True):
                check_that("committee_frozen_balance_amount", int(balance["amount"]), is_integer(), quiet=True)
                if not base_test.type_validator.is_asset_id(balance["asset_id"]):
            ...
            if "unfreeze_time" in frozen_balance_object:
                if not base_test.type_validator.is_iso8601(frozen_balance_object["unfreeze_time"]):
                    lcc.log_error(
                        "Wrong format of 'unfreeze_time', got: {}".format(frozen_balance_object["unfreeze_time"])
                    )
                else:
    ...
    def validate_committee_frozen_balance_object(self, base_test, committee_frozen_balance_object):
        ...
        if check_that("committee frozen balance", committee_frozen_balance_object, has_length(is_in([4, 5])),
            ...
            if check_that("balance", balance, has_length(2), quiet=True):
                ...
                if not base_test.type_validator.is_asset_id(balance["asset_id"]):
                    lcc.log_error(
                        "Wrong format of 'asset_id', got: {}".format(committee_frozen_balance_object["asset_id"])
                    )
                else:
                    lcc.log_info("'asset_id' has correct format")
            if not base_test.type_validator.is_committee_frozen_balance_id(committee_frozen_balance_object["id"]):
                lcc.log_error(
                    "Wrong format of 'committee_frozen_balance_id', got: {}".format(
            ...
            if not base_test.type_validator.is_committee_member_id(committee_frozen_balance_object["owner"]):
                lcc.log_error(
                    "Wrong format of 'committee_member_id', got: {}".format(committee_frozen_balance_object["owner"])
                )
            else:
            ...
            if len(committee_frozen_balance_object) == 5:
                if not base_test.type_validator.is_iso8601(committee_frozen_balance_object["unfreeze_balance_time"]):
                    lcc.log_error(
                        "Wrong format of 'unfreeze_balance_time', got {}".format(
    ...
    def validate_eth_address_object(self, base_test, eth_address_object):
        if check_that("account eth address", eth_address_object, has_length(7), quiet=True):
            ...
            if not base_test.type_validator.is_hex(eth_address_object["transaction_hash"]):
                lcc.log_error(
                    "Wrong format of 'transaction_hash', got: {}".format(eth_address_object["transaction_hash"])
                )
            else:
    ...
    def validate_deposit_eth_object(self, base_test, deposit_eth_object):
        if check_that("'deposit eth'", deposit_eth_object, has_length(10), quiet=True):
            ...
            if not base_test.type_validator.is_hex(deposit_eth_object["transaction_hash"]):
                lcc.log_error(
                    "Wrong format of 'transaction_hash', got: {}".format(deposit_eth_object["transaction_hash"])
                )
            else:
                lcc.log_info("'transaction_hash' has correct format: hex")
            check_that_in(
    ...
    def validate_withdraw_eth_object(self, base_test, withdraw_eth_object):
        if check_that("'withdraw eth'", withdraw_eth_object, has_length(12), quiet=True):
            ...
            if not base_test.type_validator.is_hex(withdraw_eth_object["transaction_hash"]):
                lcc.log_error(
                    "Wrong format of 'transaction_hash', got: {}".format(withdraw_eth_object["transaction_hash"])
                )
            else:
                lcc.log_info("'transaction_hash' has correct format: hex")
            check_that_in(
                withdraw_eth_object, "withdraw_id", greater_than_or_equal_to(0), "is_approved", is_bool(), "is_sent",
                is_bool(), "approves", is_list(), "fee", is_integer(), "echo_block_number", is_integer(), "extensions",
                is_list()
            )
    ...
    def validate_erc20_token_object(self, base_test, erc20_token_object):
        if check_that("'erc20 token'", erc20_token_object, has_length(8), quiet=True):
    ...
    def validate_erc20_deposit_object(self, base_test, erc20_deposit_object):
        if check_that("'erc20 deposit'", erc20_deposit_object, has_length(10), quiet=True):
            ...
            if not base_test.type_validator.is_hex(erc20_deposit_object["transaction_hash"]):
                lcc.log_error(
                    "Wrong format of 'transaction_hash', got: {}".format(erc20_deposit_object["transaction_hash"])
                )
            else:
                lcc.log_info("'transaction_hash' has correct format: hex")
    ...
    def validate_erc20_withdraw_object(self, base_test, erc20_withdraw_object):
        if check_that("'erc20 withdrawal'", erc20_withdraw_object, has_length(12), quiet=True):
            ...
            if not base_test.type_validator.is_erc20_object_id(erc20_withdraw_object["erc20_token"]):
                lcc.log_error("Wrong format of 'erc20_token', got: {}".format(erc20_withdraw_object["erc20_token"]))
            else:
    ...
    def validate_global_properties_object(self, base_test, global_properties_object):
        ...
        def validate_sidechain_config(base_test, sidechain_config, eth_params, eth_methods):
            if check_that("sidechain config", sidechain_config, has_length(24), quiet=True):
                for eth_param in eth_params:
                    if not base_test.type_validator.is_hex(sidechain_config[eth_param]):
                        lcc.log_error("Wrong format of '{}', got: {}".format(eth_param, sidechain_config[eth_param]))
                    else:
                        lcc.log_info("'{}' has correct format: hex".format(eth_param))
                for eth_method in eth_methods:
                    if check_that("eth_method", sidechain_config[eth_method], has_length(2), quiet=True):
                        ...
                        check_that_in(sidechain_config[eth_method], "gas", is_integer(), quiet=True)
                ...
                if check_that("fines", sidechain_config["fines"], has_length(1), quiet=True):
                    check_that_in(sidechain_config["fines"], "create_eth_address", is_integer(), quiet=True)
                check_that_in(
                    sidechain_config,
                    "satoshis_per_byte",
                    is_integer(),
                ...
                base_test.check_uint64_numbers(sidechain_config, "gas_price", quiet=True)
                if check_that("eth_update_contract_address", sidechain_config["eth_update_contract_address"],
                              has_length(2), quiet=True):
                    check_that_in(sidechain_config["eth_update_contract_address"], "method", is_str(), quiet=True)
                    check_that_in(sidechain_config["eth_update_contract_address"], "gas", is_integer(), quiet=True)

        def validate_erc20_config(base_test, erc20_config, erc20_methods):
            if check_that("erc20 config", erc20_config, has_length(6), quiet=True):
                ...
                check_that_in(erc20_config, "create_token_fee", is_integer(), quiet=True)
                for erc20_method in erc20_methods:
                    if check_that("erc20_method", erc20_config[erc20_method], has_length(2), quiet=True):
                        if not base_test.type_validator.is_hex(erc20_config[erc20_method]["method"]):
                            lcc.log_error(
                                "Wrong format of '{}', got: {}".format(erc20_method, erc20_config[erc20_method])
                            )
                        else:
                            lcc.log_info("'{}' has correct format: hex".format(erc20_method))
                        check_that_in(erc20_config[erc20_method], "gas", is_integer(), quiet=True)

        def validate_economy_config(economy_config):
            if check_that("economy config", economy_config, has_length(5), quiet=True):
                check_that_in(
                    economy_config,
                    "blocks_in_interval",
                    is_integer(),
        ...
        def no_fee(actual_fee):
            check_that("fee", actual_fee, has_length(0), quiet=True)

        def only_fee(actual_fee):
            if check_that("fee", actual_fee, has_length(1), quiet=True):
                check_that_in(actual_fee, "fee", is_integer(), quiet=True)

        def fee_with_price_per_kbyte(actual_fee):
            if check_that("fee", actual_fee, has_length(2), quiet=True):
                check_that_in(actual_fee, "fee", is_integer(), "price_per_kbyte", is_integer(), quiet=True)

        def account_create_fee(actual_fee):
            if check_that("fee", actual_fee, has_length(3), quiet=True):
                check_that_in(
                    actual_fee,
                    "basic_fee",
                    is_integer(),
        ...
        def asset_create_fee(actual_fee):
            if check_that("fee", actual_fee, has_length(4), quiet=True):
                check_that_in(
                    actual_fee,
                    "symbol3",
                    is_integer(),
        ...
        def pool_fee(actual_fee):
            if check_that("fee", actual_fee, has_length(2), quiet=True):
                check_that_in(actual_fee, "fee", is_integer(), "pool_fee", is_integer(), quiet=True)
            ...
            for operation in operations:
                for fee in current_fees:
                    if fee[0] == base_test.all_operations.get(operation.upper()):
                        lcc.log_info(
                            "Check default fee for '{}' operation, "
        ...
        fee_with_price_per_kbyte_operations = [
            "account_update", "asset_update", "proposal_create", "proposal_update", "account_address_create"
        ]
        only_fee_operations = [
            "transfer", "account_whitelist", "asset_update_bitasset", "balance_freeze", "asset_update_feed_producers",
            "asset_issue", "asset_reserve", "asset_fund_fee_pool", "asset_publish_feed", "proposal_delete",
            "committee_member_create", "committee_member_update", "committee_member_update_global_parameters",
            "vesting_balance_create", "vesting_balance_withdraw", "override_transfer", "asset_claim_fees",
            "contract_create", "contract_call", "transfer_to_address", "contract_update",
            ...
            "sidechain_erc20_deposit_token", "sidechain_erc20_withdraw_token", 'committee_member_activate',
            'committee_member_deactivate', 'committee_frozen_balance_deposit', 'committee_frozen_balance_withdraw',
            'sidechain_eth_approve_address', 'sidechain_issue', 'sidechain_burn', 'sidechain_erc20_issue',
            'sidechain_erc20_burn', 'sidechain_btc_create_address', 'sidechain_btc_create_intermediate_deposit',
            'sidechain_btc_intermediate_deposit', 'sidechain_btc_deposit', 'sidechain_btc_withdraw',
            'sidechain_btc_approve_withdraw', 'sidechain_btc_aggregate', 'sidechain_erc20_approve_token_withdraw',
            'sidechain_eth_send_deposit', 'sidechain_eth_send_withdraw', 'sidechain_eth_update_contract_address',
            'sidechain_erc20_send_deposit_token', 'sidechain_erc20_send_withdraw_token', 'did_create', 'did_update',
            'did_delete', '', 'request_unfreeze_balance'
        ]
        no_fee_operations = [
            "balance_claim", "balance_unfreeze", 'contract_internal_create', 'contract_internal_call',
            'contract_selfdestruct', 'evm_address_register_operation'
        ]
        ...
        if check_that("parameters", parameters, has_length(25), quiet=True):
            ...
            if check_that("current fees", current_fees, has_length(2), quiet=True):
                check_that_in(current_fees, "parameters", is_list(), "scale", is_integer(), quiet=True)
            ...
            for frozen_balance in frozen_balances_multipliers:
                for balance in frozen_balance:
                    check_that("frozen_balance", balance, is_integer(), quiet=True)

            lcc.set_step("Check the count of fees for operations")
            fee_parameters = current_fees["parameters"]
            require_that("count of fees for operations", fee_parameters, has_length(70))

            lcc.set_step("Check that count of checking fees fields equal to all operations")
            checking_operations_fee_types = [
                fee_with_price_per_kbyte_operations, only_fee_operations, no_fee_operations,
                account_create_fee_operations, asset_create_fee_operations, pool_fee_operations
            ]
            all_checking_operations = []
            for fee_type in checking_operations_fee_types:
                all_checking_operations.extend(fee_type)
            check_that(
                "'length of checking fees fields equal to all operations'", all_checking_operations, has_length(70)
            )
        ...
        for fee_parameter in fee_parameters:
            ...
            if len(parameter) == 3 and ("basic_fee" and "premium_fee" and "price_per_kbyte") in parameter:
                ...
                continue
            if len(parameter) == 4 and ("symbol3" and "symbol4" and "long_symbol" and "price_per_kbyte") in parameter:
                asset_create_fee_operations_ids.append(fee_parameter[0])
                base_test.asset_create_count += 1
                continue
            else:
                lcc.log_warning(
                    "Warn: Added new option for calculating fee for the operation, got: '{}'".format(fee_parameter)
                )
        ...
        check_that(
            ...
            fee_with_price_per_kbyte_operations,
            has_length(base_test.fee_with_price_per_kbyte_count),
            quiet=True
        )
        check_default_fee_for_operation(
            base_test, fee_parameters, fee_with_price_per_kbyte_operations, fee_with_price_per_kbyte
        )
        show_wrong_fee_operations(
            base_test, fee_with_price_per_kbyte_operations_ids, fee_with_price_per_kbyte_operations
        )

        lcc.set_step("Check 'only_fee' for operations")
        check_that("'only_fee' operation count", only_fee_operations, has_length(base_test.only_fee_count), quiet=True)
        check_default_fee_for_operation(base_test, fee_parameters, only_fee_operations, only_fee)
        show_wrong_fee_operations(base_test, only_fee_operations_ids, only_fee_operations)

        lcc.set_step("Check 'no_fee' for operations")
        check_that("'no_fee' operation count", no_fee_operations, has_length(base_test.no_fee_count), quiet=True)
        check_default_fee_for_operation(base_test, fee_parameters, no_fee_operations, no_fee)
        show_wrong_fee_operations(base_test, no_fee_operations_ids, no_fee_operations)

        lcc.set_step("Check 'account_create_fee' for operations")
        check_that(
            "'account_create_fee' operation count",
            account_create_fee_operations,
            has_length(base_test.account_create_fee_count),
            quiet=True
        )
        check_default_fee_for_operation(base_test, fee_parameters, account_create_fee_operations, account_create_fee)
        show_wrong_fee_operations(base_test, account_create_fee_operations_ids, account_create_fee_operations)

        lcc.set_step("Check 'asset_create_fee' for operations")
        check_that(
            "'asset_create_fee' operation count",
            asset_create_fee_operations,
            has_length(base_test.asset_create_count),
            quiet=True
        )
        check_default_fee_for_operation(base_test, fee_parameters, asset_create_fee_operations, asset_create_fee)
        show_wrong_fee_operations(base_test, asset_create_fee_operations_ids, asset_create_fee_operations)

        lcc.set_step("Check 'pool_fee' for operations")
        check_that("'pool_fee' operation count", pool_fee_operations, has_length(base_test.pool_fee_count), quiet=True)
        if not check_default_fee_for_operation(base_test, fee_parameters, pool_fee_operations, pool_fee):
            show_wrong_fee_operations(base_test, pool_fee_operations_ids, pool_fee_operations)

        lcc.set_step("Check global parameters: 'echorand_config' field")
        echorand_config = parameters["echorand_config"]
        if check_that("echorand_config", echorand_config, has_length(9), quiet=True):
            check_that_in(
                echorand_config,
                "_time_generate",
                is_integer(),
        ...
        eth_params = [
            "eth_contract_address", "eth_committee_updated_topic", "eth_gen_address_topic", "eth_deposit_topic",
            "eth_withdraw_topic", "erc20_deposit_topic", "erc20_withdraw_topic"
        ]
        eth_methods = [
            "eth_committee_update_method", "eth_gen_address_method", "eth_withdraw_method", "eth_update_addr_method",
            "eth_withdraw_token_method", "eth_collect_tokens_method"
        ]
        ...
        if check_that("gas price", gas_price, has_length(2), quiet=True):
            check_that_in(gas_price, "price", is_integer(), "gas_amount", is_integer(), quiet=True)
    ...
    def validate_dynamic_global_property_object(self, base_test, dynamic_global_property_object):
        ...
        if check_that("dynamic global properties", dynamic_global_property_object, has_length(11), quiet=True):
            ...
            for property_ in dynamic_global_properties:
                base_test.check_uint64_numbers(dynamic_global_property_object, property_, quiet=True)
                value = int(dynamic_global_property_object[property_])
                check_that(property_, value, greater_than_or_equal_to(0), quiet=True)
                ...
                if not base_test.type_validator.is_iso8601(dynamic_global_property_object[time_property]):
                    lcc.log_error(
                        "Wrong format of '{}', got: {}".format(
                            time_property, dynamic_global_property_object[time_property]
                        )
                    )
                else:
                    lcc.log_info("'{}' has correct format: iso8601".format(time_property))
            check_that_in(dynamic_global_property_object, "extensions", is_list(), quiet=True)

    def validate_chain_properties_object(self, base_test, chain_properties_object):
        if check_that("chain properties", chain_properties_object, has_length(3), quiet=True):
            ...
            check_that_in(chain_properties_object, "extensions", is_list(), quiet=True)

    def validate_asset_dynamic_data_object(self, base_test, asset_dynamic_data_object):
        if check_that("asset dynamic data", asset_dynamic_data_object, has_length(5), quiet=True):
    ...
    def validate_asset_bitasset_data_object(self, base_test, asset_bitasset_data_object):
        ...
        def validate_price_object(base_test, price_object):
            check_that_in(price_object, "base", is_dict(), "quote", is_dict(), quiet=True)
            for key in price_object:
                price_part = price_object[key]
                base_test.check_uint64_numbers(price_part, "amount", quiet=True)
                if not base_test.type_validator.is_asset_id(price_part["asset_id"]):
                    lcc.log_error("Wrong format of {} 'asset_id', got: {}".format(key, price_part["asset_id"]))
                else:
                    lcc.log_info("{} 'asset_id' has correct format: asset_id".format(key))

        if check_that("asset bitasset data", asset_bitasset_data_object, has_length(6), quiet=True):
            ...
            if not base_test.type_validator.is_iso8601(asset_bitasset_data_object["current_feed_publication_time"]):
                lcc.log_error(
                    "Wrong format of 'current_feed_publication_time', got: {}".format(
            ...
            if check_that_in(asset_bitasset_data_object, "feeds", is_list(), "options", is_dict(), "extensions",
                             is_list(), quiet=True):
                options = asset_bitasset_data_object["options"]
                if check_that("options", options, has_length(4), quiet=True):
                    if not base_test.type_validator.is_asset_id(options["short_backing_asset"]):
                        lcc.log_error(
                            "Wrong format of 'short_backing_asset', got: {}".format(options["short_backing_asset"])
                        )
                    else:
    ...
    def validate_account_balance_object(self, base_test, account_balance_object):
        if check_that("account balance object", account_balance_object, has_length(5), quiet=True):
            ...
            check_that_in(account_balance_object, "balance", is_integer(), "extensions", is_list(), quiet=True)

    def validate_account_statistics_object(self, base_test, account_statistics_object):
        if check_that("account statistics object", account_statistics_object, has_length(10), quiet=True):
            ...
            if not base_test.type_validator.is_account_transaction_history_id(
                ...
                lcc.log_error(
                    "Wrong format of 'most_recent_op', got: {}".format(account_statistics_object["most_recent_op"])
                )
            else:
    ...
    def validate_block_summary_object(self, base_test, block_summary_object):
        if check_that("block summary object", block_summary_object, has_length(3), quiet=True):
            ...
            check_that_in(block_summary_object, "extensions", is_list(), quiet=True)

    def validate_account_address_object(self, base_test, account_address_object):
        if check_that("account addresses object", account_address_object, has_length(5), quiet=True):
            ...
            check_that_in(account_address_object, "label", is_str(), "extensions", is_list(), quiet=True)

    def validate_contract_balance_object(self, base_test, contract_balance_object):
        if check_that("contract balance object", contract_balance_object, has_length(5), quiet=True):
            ...
            if not base_test.type_validator.is_asset_id(contract_balance_object["asset_type"]):
                lcc.log_error("Wrong format of 'asset_type', got: {}".format(contract_balance_object["asset_type"]))
            else:
                lcc.log_info("'asset_type' has correct format: asset_id")
            check_that_in(contract_balance_object, "balance", is_integer(), "extensions", is_list(), quiet=True)
    ...
    def validate_contract_history_object(self, base_test, contract_history_object):
        if len(contract_history_object) == 7:
            ...
            if check_that("contract history object", contract_history_object, has_length(6), quiet=True):
                ...
                if not base_test.type_validator.is_operation_history_id(contract_history_object["operation_id"]):
                    lcc.log_error(
                        "Wrong format of 'operation_id', got: {}".format(contract_history_object["operation_id"])
                    )
                else:
                ...
                check_that_in(contract_history_object, "sequence", is_integer(), "extensions", is_list(), quiet=True)

    def validate_contract_statistics_object(self, base_test, contract_statistics_object):
        if check_that("contract statistics object", contract_statistics_object, has_length(6), quiet=True):
            ...
            if not base_test.type_validator.is_contract_history_id(contract_statistics_object["most_recent_op"]):
                lcc.log_error(
                    "Wrong format of 'most_recent_op', got: {}".format(contract_statistics_object["most_recent_op"])
                )
            else:
