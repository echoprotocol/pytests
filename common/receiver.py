...
class Receiver(object):
    ...
    @staticmethod
    def get_positive_result(response):
        lcc.log_info("Received:\n{}".format(json.dumps(response, indent=4)))
        return response

    @staticmethod
    def get_negative_result(response):
        lcc.log_warn("Error received:\n{}".format(json.dumps(response, indent=4)))
        return response

    def get_response(self, id_response, positive):
        response = json.loads(self.web_socket.recv())

        if response.get("method") == "notice":
            return self.get_response(id_response, positive)
        if response.get("id") != id_response:
            return lcc.log_error(
                "Wrong 'id' expected '{}', but received:\n{}".format(id_response, json.dumps(response, indent=4)))
        if response.get("jsonrpc") != "2.0":
            return lcc.log_error("Wrong data received: {}".format(json.dumps(response, indent=4)))
        if positive:
            return self.get_positive_result(response)
        return self.get_negative_result(response)

    def get_notice_obj(self, response, expected_id, log_block_id):
    ...
    def get_notice(self, id_response, object_id, log_block_id):
        response = json.loads(self.web_socket.recv())
        ...
        if (object_id is not None) and (self.validator.is_object_id(response.get("params")[1][0][0]["id"])):
            return self.get_notice_obj(response, object_id, log_block_id)
