...
from project import GAS_PRICE, MIN_ETH_WITHDRAW, MIN_ETH_WITHDRAW_FEE
...
class Ethereum(BaseTest):
    ...
    def withdraw_eth_to_ethereum_address(self, from_account, withdraw_amount):
        ...
        eth_address_balance = self.eth_trx.get_address_balance_in_eth_network(
            self.web3, self.eth_address, currency="wei"
        )
        ...
        self.utils.perform_sidechain_eth_withdraw_operation(
            self, from_account, self.eth_address, withdraw_amount, self.__database_api_identifier
        )
        ...
        updated_address_balance_in_ethereum = int(eth_address_balance) +\
            self.utils.convert_eeth_to_currency(withdraw_amount, currency="wei") - self.withdraw_fee
        lcc.set_step("Check that withdraw eth completed successfully")
        check_that(
            "'updated address balance in ethereum'", int(eth_address_balance_after_withdraw),
            equal_to(updated_address_balance_in_ethereum)
        )
    ...
    def ethereum_sidechain_pre_run_scenario(self, get_random_valid_account_name):
        ...
        self.utils.perform_sidechain_eth_create_address_operation(
            self, self.new_account, self.__database_api_identifier
        )
    ...
    def ethereum_in_scenario(self):
        ...
        lcc.log_info(
            "Unpaid fee for creation ethereum address for '{}' account: '{}'".format(
                self.new_account, unpaid_fee_in_ethereum
            )
        )
        ...
        ethereum_balance_first_in = int(
            self.utils.get_eth_balance(self, self.new_account, self.__database_api_identifier)
        )

        check_that(
            "'updated balance in ethereum'", ethereum_balance_first_in,
        ...
        transaction = self.eth_trx.get_transfer_transaction(
            web3=self.web3, _from=self.eth_address, _to=self.eth_account_address, value=eth_amount
        )
        self.eth_trx.broadcast(web3=self.web3, transaction=transaction)
        ...
        ethereum_balance_second_in = int(
            self.utils.get_eth_balance(
                self, self.new_account, self.__database_api_identifier, ethereum_balance_first_in
            )
        )
        check_that(
            "'updated balance in ethereum'", ethereum_balance_second_in,
    ...
    def transfer_eeth_scenario(self):
        ...
        recipient_balance = int(
            self.utils.get_account_balances(self, self.echo_acc0, self.__database_api_identifier,
        ...
        check_that(
            "'updated balance in ethereum'", recipient_balance_after_transfer,
    ...
    def transfer_eeth_to_account_address_scenario(self, get_random_string):
        ...
        for i in range(addresses_count):
            self.utils.perform_account_address_create_operation(
                self, self.echo_acc0, label + str(i), self.__database_api_identifier
            )
        ...
        recipient_balance_after_transfer = int(
            self.utils.get_eth_balance(self, self.echo_acc0, self.__database_api_identifier, recipient_balance)
        )
        lcc.log_info(
            "Account '{}' balance after transfer in ethereum is '{}'".format(self.new_account, ethereum_balance)
        )
        lcc.log_info(
            "Recipient '{}' balance after transfer in ethereum is '{}'".format(
        ...
        check_that(
            "'updated recipient balance in ethereum'", recipient_balance_after_transfer,
            equal_to(recipient_balance + transfer_amount)
        )

        lcc.set_step("Transfer assets via second account_address")
        transfer_amount = self.get_random_amount(
            _to=account_balance_after_transfer - MIN_ETH_WITHDRAW_FEE, amount_type=int
        )
        self.utils.perform_transfer_to_address_operations(
            self,
            self.new_account,
        ...
        time.sleep(5)
        recipient_balance_after_second_transfer = int(
            self.utils.get_eth_balance(
                self, self.echo_acc0, self.__database_api_identifier, recipient_balance_after_transfer
            )
        )
        lcc.log_info(
            "Recipient '{}' balance after second "
        ...
        check_that(
            "'updated recipient balance in ethereum'", recipient_balance_after_second_transfer,
