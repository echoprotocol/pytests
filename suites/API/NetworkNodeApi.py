...
from lemoncheesecake.matching import (
    check_that, check_that_in, equal_to, has_length, is_integer, is_list, is_none, is_true, require_that
)
...
class NetworkNodeApi(BaseTest):
    ...
    def connection_to_network_node_api(self):
        ...
        if require_that("'call method 'get_info''", result, has_length(6), quiet=True):
            check_that_in(
                result, "listening_on", equal_to(self.endpoint), "accept_incoming_connections", is_true(), "firewalled",
                equal_to("unknown"), "connection_count", equal_to(0)
            )
        if not self.type_validator.is_privkey(result["node_public_key"]):
            lcc.log_error("Wrong format of 'node_public_key', got: '{}'".format(result["node_public_key"]))
        else:
    ...
    def add_node(self):
        ...
        check_that("'call method 'add_node''", response["result"], is_none(), quiet=True)
    ...
    def get_connected_peers(self):
        ...
        response = self.get_response(response_id)
        check_that("'call method 'add_node''", response["result"], is_list(), quiet=True)
    ...
    def get_potential_peers(self):
        ...
        if require_that("'call method 'get_potential_peers''", result, has_length(6), quiet=True):
            check_that_in(
                result, "endpoint", equal_to(self.endpoint), "last_connection_disposition",
                equal_to("last_connection_handshaking_failed"), "number_of_successful_connection_attempts",
                is_integer(), "number_of_failed_connection_attempts", is_integer()
            )
            if not self.type_validator.is_iso8601(result["last_seen_time"]):
                lcc.log_error("Wrong format of 'last_seen_time', got: {}".format(result["last_seen_time"]))
            else:
    ...
    def set_advanced_node_parameters(self):
        ...
        result = self.get_response(response_id)["result"]
        check_that("'call method 'add_node''", result, is_none(), quiet=True)
