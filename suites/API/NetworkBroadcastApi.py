...
class NetworkBroadcastApi(object):
    ...
    @lcc.depends_on(
        "API.DatabaseApi.DatabaseApi.connection_to_database_api",
        "API.RegistrationApi.RegistrationApi.connection_to_registration_api"
    )
    def connection_to_network_broadcast_api(self):
        ...
        transfer_operation = base.echo_ops.get_transfer_operation(
            echo=base.echo, from_account_id=echo_acc0, to_account_id=echo_acc1
        )
        collected_operation = base.collect_operations(transfer_operation, database_api_identifier)
        transaction_object = base.echo_ops.broadcast(
            echo=base.echo, list_operations=collected_operation, no_broadcast=True
        )
        ...
        response_id = base.send_request(
            base.get_request("broadcast_transaction", [transaction_object]), api_identifier + 1
        )
