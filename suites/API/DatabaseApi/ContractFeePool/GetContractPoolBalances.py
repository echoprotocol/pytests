...
from lemoncheesecake.matching import check_that, check_that_in, equal_to, has_length, require_that_in
...
class GetContractPoolBalances(BaseTest):
    ...
    def method_main_check(self, get_random_integer):
        ...
        if check_that("contract pool balance", result, has_length(2)):
            check_that_in(result, "amount", equal_to(value_to_pool), "asset_id", equal_to(self.echo_asset))
...
class PositiveTesting(BaseTest):
    ...
    def add_fee_pool_to_call_contract(self, get_random_valid_account_name, get_random_integer_up_to_ten):
        ...
        contract_id = self.utils.get_contract_id(
            self, self.echo_acc0, self.contract, self.__database_api_identifier, value_amount=contract_balance
        )
        ...
        require_that_in(account_balance, "amount", equal_to(0), "asset_id", equal_to(self.echo_asset))
        ...
        operation = self.echo_ops.get_contract_call_operation(
            echo=self.echo, registrar=new_account, bytecode=self.greet, callee=contract_id
        )
        ...
        operation = self.echo_ops.get_contract_call_operation(
            echo=self.echo, registrar=new_account, bytecode=self.get_pennie, callee=contract_id
        )
        needed_fee = self.get_required_fee(operation, self.__database_api_identifier)["amount"]
        self.utils.perform_transfer_operations(
            self, self.echo_acc0, new_account, self.__database_api_identifier, transfer_amount=needed_fee
        )
        ...
        self.utils.perform_contract_fund_pool_operation(
            self, new_account, contract_id, needed_fee, self.__database_api_identifier
        )
        lcc.log_info("Added '{}' assets value to '{}' contract fee pool successfully".format(needed_fee, contract_id))
    ...
    def add_fee_pool_and_destroy_contract(self, get_random_integer_up_to_ten):
        ...
        self.utils.perform_contract_fund_pool_operation(
            self, self.account_id, contract_id, value_to_pool, self.__database_api_identifier
        )
        lcc.log_info("Fee pool added to '{}' contract successfully".format(contract_id))
        ...
        check_that(
            "'account balance'", int(updated_account_balance),
    ...
    def add_insufficient_fee_pool_to_call_contract(self):
        ...
        lcc.log_info(
            "Amount '{}' in '{}' assets added to new account '{}'".format(
                needed_fee + value_to_pool, self.echo_asset, self.account_id
            )
        )
...
class NegativeTesting(BaseTest):
    ...
    @lcc.depends_on("API.DatabaseApi.ContractFeePool.GetContractPoolBalances.GetContractPoolBalances.method_main_check")
    def add_fee_pool_in_not_echo_asset(
        self, get_random_valid_asset_name, get_random_integer, get_random_integer_up_to_ten
    ):
        ...
        self.utils.add_assets_to_account(
            self, asset_value, new_asset_id, self.echo_acc0, self.__database_api_identifier
        )
