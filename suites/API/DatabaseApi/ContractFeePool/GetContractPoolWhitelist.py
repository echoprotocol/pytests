...
from lemoncheesecake.matching import check_that, check_that_in, equal_to, has_entry, has_length, is_list, require_that
...
class GetContractPoolWhitelist(BaseTest):
    ...
    def method_main_check(self, get_random_integer):
        ...
        self.utils.perform_contract_fund_pool_operation(
            self, self.echo_acc0, contract_id, value_to_pool, self.__database_api_identifier
        )
        ...
        if check_that("contract pool whitelist", result, has_length(2)):
            check_that_in(result, "whitelist", is_list([]), "blacklist", is_list([]))
        ...
        if check_that("contract pool whitelist", result, has_length(2)):
            check_that_in(result, "whitelist", is_list(whitelist), "blacklist", is_list(blacklist))
...
class PositiveTesting(BaseTest):
    ...
    def add_remove_accounts_to_from_whitelist(self):
        ...
        self.utils.perform_contract_fund_pool_operation(
            self, self.echo_acc0, contract_id, start_fee_pool_balance, self.__database_api_identifier
        )
        lcc.log_info(
            "Added '{}' assets value to '{}' contract fee pool successfully".format(
                start_fee_pool_balance, contract_id
            )
        )
        ...
        self.utils.perform_contract_whitelist_operation(
            self, self.echo_acc0, contract_id, self.__database_api_identifier, add_to_whitelist=whitelist
        )
        ...
        account_balance = self.utils.get_account_balances(self, self.echo_acc0,
        ...
        account_balance_after_call = self.utils.get_account_balances(
            self, self.echo_acc0, self.__database_api_identifier
        )["amount"]
        ...
        self.utils.perform_contract_whitelist_operation(
            self, self.echo_acc0, contract_id, self.__database_api_identifier, remove_from_whitelist=full_whitelist
        )
        ...
        operation_method = self.echo_ops.get_contract_call_operation(
            echo=self.echo, registrar=self.echo_acc5, bytecode=self.greet, callee=contract_id
        )
        collected_operation = self.collect_operations(operation_method, self.__database_api_identifier)
        self.echo_ops.broadcast(echo=self.echo, list_operations=collected_operation, log_broadcast=False)
    ...
    def add_remove_accounts_to_from_blacklist(self):
        ...
        needed_fee = self.get_required_fee(operation_method, self.__database_api_identifier)["amount"]
        self.utils.perform_contract_fund_pool_operation(
            self, self.echo_acc0, contract_id, needed_fee, self.__database_api_identifier
        )
        ...
        for account in blacklist:
            operation_method = self.echo_ops.get_contract_call_operation(
                echo=self.echo, registrar=account, bytecode=self.greet, callee=contract_id
            )
            collected_operation = self.collect_operations(operation_method, self.__database_api_identifier)
            try:
                self.echo_ops.broadcast(echo=self.echo, list_operations=collected_operation)
                lcc.log_error(
                    "Error: broadcast transaction complete when '{}' don't have enough founds to pay fee"
                    .format(account)
                )
        ...
        for account in blacklist:
            ...
            full_blacklist = sorted(full_blacklist, key=self.get_value_for_sorting_func)
        self.utils.perform_contract_whitelist_operation(
            self, self.echo_acc0, contract_id, self.__database_api_identifier, add_to_blacklist=blacklist
        )
        ...
        self.utils.perform_contract_whitelist_operation(
            self, self.echo_acc0, contract_id, self.__database_api_identifier, remove_from_blacklist=full_blacklist
        )
...
class NegativeTesting(BaseTest):
    ...
    def add_twice_account_and_remove_in_the_same_time(self, get_random_integer):
        ...
        try:
            ...
            lcc.log_error("Error: broadcast transaction complete. Can add account to whitelist twice")
        except RPCError as e:
            lcc.log_info(str(e))
        ...
        try:
            self.utils.perform_contract_whitelist_operation(
                self, self.echo_acc0, contract_id, self.__database_api_identifier, remove_from_whitelist=whitelist
            )
            lcc.log_error("Error: broadcast transaction complete. Can remove account from whitelist twice")
        except RPCError as e:
            lcc.log_info(str(e))
        ...
        try:
            ...
            lcc.log_error("Error: broadcast transaction complete. Can add account to blacklist twice")
        except RPCError as e:
            lcc.log_info(str(e))
        ...
        try:
            self.utils.perform_contract_whitelist_operation(
                self, self.echo_acc0, contract_id, self.__database_api_identifier, remove_from_blacklist=blacklist
            )
            lcc.log_error("Error: broadcast transaction complete. Can remove account from blacklist twice")
        except RPCError as e:
            lcc.log_info(str(e))
