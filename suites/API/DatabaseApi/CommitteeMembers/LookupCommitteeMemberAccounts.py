...
from fixtures.base_fixtures import get_random_btc_public_key, get_random_eth_address

import lemoncheesecake.api as lcc
from lemoncheesecake.matching import check_that, equal_to, has_length, is_true, require_that
...
class LookupCommitteeMemberAccounts(BaseTest):
    ...
    def method_main_check(self):
        ...
        active_committee_members = self.get_response(
            response_id, log_response=True
        ...
        response_id = self.send_request(
            self.get_request("get_objects", [active_committee_members_ids]), self.__database_api_identifier
        )
        ...
        response_id = self.send_request(
            self.get_request("get_accounts", [committee_member_accounts]), self.__database_api_identifier
        )
        ...
        for i, account_name in enumerate(account_names):
            ...
            response_id = self.send_request(
                self.get_request("lookup_committee_member_accounts", [account_name, limit]),
                self.__database_api_identifier
            )
            committee_member_result = self.get_response(response_id)["result"][0]
            lcc.log_info("Call method 'lookup_committee_member_accounts' with params='{}'".format(account_name))
...
class PositiveTesting(BaseTest):
    ...
    @lcc.depends_on(
        "API.DatabaseApi.CommitteeMembers.LookupCommitteeMemberAccounts.LookupCommitteeMemberAccounts.method_main_check"
    )
    def lookup_info_about_created_committee_members(self, get_random_valid_account_name):
        ...
        response_id = self.send_request(
            self.get_request("lookup_committee_member_accounts", [valid_account_name, account_count]),
            self.__database_api_identifier
        )
        committee_members_info = self.get_response(response_id)["result"]
        lcc.log_info(
            "Call method 'lookup_committee_member_accounts' with params: lower_bound_name='{}', limit='{}'".format(
    ...
    def lookup_more_accounts_than_exists(self):
        ...
        lower_bound_name, limit = self.utils.get_nonexistent_account_name(
            self, self.__database_api_identifier, committee_member=True
        )
        ...
        response_id = self.send_request(
            self.get_request("lookup_committee_member_accounts", [lower_bound_name, limit]),
            self.__database_api_identifier
        )
    ...
    def check_alphabet_order_in_cut_committee_members_accounts_lookup_info(self, get_random_valid_account_name):
        ...
        new_account_id = self.get_account_id(
            valid_account_name, self.__database_api_identifier, self.__registration_api_identifier
        )
        lcc.log_info("New Echo account created, account_id='{}'".format(new_account_id))

        lcc.set_step("Lookup committee member accounts")
        response_id = self.send_request(
            self.get_request("lookup_committee_member_accounts", [valid_account_name, limit]),
            self.__database_api_identifier
        )
        accounts = self.get_response(response_id)["result"]
        lcc.log_info(
