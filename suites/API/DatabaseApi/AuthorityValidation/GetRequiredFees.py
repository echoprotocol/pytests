...
from lemoncheesecake.matching import (
    check_that, check_that_in, has_entry, is_, is_dict, is_integer, is_not_none, require_that_in
)
...
class GetRequiredFees(BaseTest):
    ...
    def method_main_check(self):
        ...
        for result in results:
            check_that_in(result, "amount", is_integer(), "asset_id", is_(self.echo_asset), quiet=True)
...
class PositiveTesting(BaseTest):
    ...
    def setup_suite(self):
        ...
        self.transfer_operation = self.echo_ops.get_transfer_operation(
            echo=self.echo, from_account_id=self.echo_acc0, to_account_id=self.echo_acc1, amount=self.amount
        )
    ...
    def get_required_fee_in_eth_asset(self):
        ...
        lcc.set_step("Check simple work of method 'get_required_fees'")
        for result in results:
            check_that_in(result, "amount", is_integer(), "asset_id", is_(self.eth_asset), quiet=True)
    ...
    def get_required_fee_in_btc_asset(self):
        ...
        lcc.set_step("Check simple work of method 'get_required_fees'")
        for result in results:
            check_that_in(result, "amount", is_integer(), "asset_id", is_(self.btc_asset), quiet=True)
    ...
    def fee_equal_to_get_required_fee(self):
        ...
        self.add_fee_to_operation(
            self.transfer_operation, self.__database_api_identifier, fee_amount=(self.required_fee.get("amount"))
        )
        broadcast_result = self.echo_ops.broadcast(
            echo=self.echo, list_operations=self.transfer_operation, log_broadcast=False
        )
        check_that("broadcast transaction complete successfully", broadcast_result["trx"], is_not_none(), quiet=True)
    ...
    def fee_higher_than_get_required_fee(self):
        ...
        self.add_fee_to_operation(
            self.transfer_operation, self.__database_api_identifier, fee_amount=(self.required_fee.get("amount") + 1)
        )
        broadcast_result = self.echo_ops.broadcast(
            echo=self.echo, list_operations=self.transfer_operation, log_broadcast=False
        )
    ...
    def fee_in_eth_asset(self):
        ...
        check_that_in(response["result"][0], "amount", is_integer(), "asset_id", is_(self.eth_asset), quiet=True)
    ...
    def fee_to_call_contract(self):
        ...
        require_that_in(result, "fee", is_dict(), "user_to_pay", is_dict(), quiet=True)
        check_that_in(result["fee"], "amount", is_integer(), "asset_id", is_(self.echo_asset), quiet=True)
        check_that_in(result["user_to_pay"], "amount", is_integer(), "asset_id", is_(self.echo_asset), quiet=True)
...
class NegativeTesting(BaseTest):
    ...
    def call_method_without_params_or_insufficient(self):
        ...
        response_id = self.send_request(
            self.get_request("get_required_fees", [[self.transfer_operation_ex]]), self.__database_api_identifier
        )
        response = self.get_response(response_id, negative=True)
        check_that(
            "'get_required_fees' return error message",
    ...
    def fee_lower_than_get_required_fee(self):
        ...
        self.add_fee_to_operation(
            self.transfer_operation, self.__database_api_identifier, fee_amount=(required_fee.get("amount") - 1)
        )
        try:
    ...
    def sender_do_not_have_enough_fee(self, get_random_valid_account_name):
        ...
        account_id = self.get_account_id(
            account_name, self.__database_api_identifier, self.__registration_api_identifier
        )
        ...
        lcc.log_info("Account '{}' has '{}' in '{}' asset".format(account_id, all_balance_amount, self.echo_asset))

        lcc.set_step(
            "Send transfer transaction with a fee equal to the 'get_required_fee', "
            "but sender don't have enough fee"
        )
        operation = self.echo_ops.get_transfer_operation(
            echo=self.echo, from_account_id=account_id, to_account_id=self.echo_acc1, amount=self.amount
        )
    ...
    def nonexistent_contract_byte_code(self):
        ...
        operation = self.echo_ops.get_contract_create_operation(
            echo=self.echo, registrar=self.echo_acc0, bytecode=not_valid_contract
        )
        response = self.get_required_fees(operation, self.echo_asset, negative=True)
        check_that("'get_required_fees' return error message", response, has_entry("error"), quiet=True)
    ...
    def nonexistent_method_byte_code(self, get_random_eth_address):
        lcc.set_step("Get required fee for 'contract_call_operation' with nonexistent method byte code")
        operation = self.echo_ops.get_contract_call_operation(
            echo=self.echo, registrar=self.echo_acc0, bytecode=get_random_eth_address, callee=self.valid_contract_id
        )
        response = self.get_required_fees(operation, self.echo_asset, negative=True)
        check_that("'get_required_fees' return error message", response, has_entry("error"), quiet=True)
