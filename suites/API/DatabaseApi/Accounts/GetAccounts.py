...
@lcc.tags("api", "database_api", "database_api_accounts", "get_accounts", "database_api_objects", "get_objects")
...
class GetAccounts(BaseTest):
    ...
    def method_main_check(self):
        ...
        require_that("'list of received accounts'", get_accounts_results, has_length(len(params)), quiet=True)
...
class PositiveTesting(BaseTest):
    ...
    @lcc.depends_on("API.DatabaseApi.Accounts.GetAccounts.GetAccounts.method_main_check")
    def get_info_about_created_accounts(self, get_random_valid_account_name):
        ...
        response_id = self.send_request(
            self.get_request("get_accounts", [accounts.get("accounts_ids")]), self.__database_api_identifier
        )
        ...
        for i, account_info in enumerate(results):
            ...
            check_that_in(
                ...
                equal_to(performed_operations["name"]), "active", equal_to(performed_operations["active"]),
                "echorand_key", equal_to(performed_operations["echorand_key"]), "options",
                equal_to(performed_operations["options"])
            )
    ...
    def compare_with_method_get_objects(self, get_random_valid_account_name):
        ...
        require_that('results', get_objects_results, equal_to(get_accounts_results), quiet=True)
