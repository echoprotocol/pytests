...
class PositiveTesting(BaseTest):
    ...
    @lcc.depends_on("API.DatabaseApi.Accounts.GetAccountAddresses.GetAccountAddresses.method_main_check")
    def check_from_and_limit_param(
        self, get_random_valid_account_name, get_random_string, get_random_integer_up_to_hundred
    ):
        new_account = get_random_valid_account_name
        ...
        self.utils.perform_account_address_create_operation(
            self, new_account, label, self.__database_api_identifier, operation_count=total_addresses_count
        )
        lcc.log_info(
            "Account address create operation for new account performed. '{}' addresses created"
            .format(total_addresses_count)
        )
        ...
        response = self.get_account_addresses(new_account)
        lcc.log_info(
            "Call method 'get_account_addresses' of new account, with from='{}', limit='{}' parameters".format(
                _from, limit
            )
        )
