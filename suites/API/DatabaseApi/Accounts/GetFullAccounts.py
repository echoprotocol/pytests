...
from lemoncheesecake.matching import check_that_in, check_that, has_length, is_str, is_list, \
    require_that, equal_to, is_dict
...
@lcc.prop("main", "type")
@lcc.tags(
    "api", "database_api", "database_api_accounts", "get_full_accounts",
    "database_api_objects", "get_objects"
)
@lcc.suite(
    "Check work of methods: 'get_full_accounts', 'get_objects' (account balance & account statistics objects)",
    rank=1
)
class GetFullAccounts(BaseTest):
    ...
    def method_main_check(self):
        ...
        for i, result in enumerate(results):
            ...
            check_that(
                "account_id",
                result[0], equal_to(params[i]),
                quiet=True
            )
            full_account_info = result[1]
            if check_that(
                "full_account_info",
                full_account_info, has_length(7),
                quiet=True
            ):
        ...
        require_that(
            'results',
            get_objects_results[0], equal_to(balance),
            quiet=True
        )
        ...
        require_that(
            'results',
            get_objects_results[0], equal_to(account_statistics),
            quiet=True
        )
