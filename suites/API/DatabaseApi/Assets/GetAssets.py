...
from lemoncheesecake.matching import check_that, check_that_in, equal_to, has_length, is_none, require_that
...
@lcc.tags("api", "database_api", "database_api_assets", "get_assets", "database_api_objects", "get_objects")
@lcc.suite(
    "Check work of methods: 'get_assets', 'get_objects (asset & dynamic asset data & bitasset data objects)'", rank=1
)
class GetAssets(BaseTest):
    ...
    @lcc.test("Simple work of methods: 'get_assets', 'get_objects (asset & dynamic asset data)")
    def method_main_check(self):
        ...
        require_that('results', get_objects_results, equal_to(get_assets_results), quiet=True)
...
class PositiveTesting(BaseTest):
    ...
    @lcc.depends_on("API.DatabaseApi.Assets.GetAssets.GetAssets.method_main_check")
    def get_info_about_create_assets(self, get_random_valid_asset_name):
        ...
        response_id = self.send_request(self.get_request("get_assets", [asset_ids]), self.__database_api_identifier)
    ...
    def get_info_about_nonexistent_asset_id(self):
        ...
        response_id = self.send_request(
            self.get_request("get_assets", [nonexistent_asset_id]), self.__database_api_identifier
        )
        response = self.get_response(response_id)
    ...
    def get_object_for_asset_bitasset_data(self, get_random_valid_asset_name):
        ...
        response_id = self.send_request(
            self.get_request("get_assets", [[created_asset_id]]), self.__database_api_identifier
        )
        ...
        response_id = self.send_request(
            self.get_request("get_objects", [[bitasset_id]]), self.__database_api_identifier
        )
