...
from lemoncheesecake.matching import check_that, equal_to, is_false, is_true, require_that
...
class CallContractNoChangingState(BaseTest):
    ...
    def method_main_check(self):
        ...
        params = [contract_id, self.echo_acc0, {
            "amount": 100,
            "asset_id": self.echo_asset
        }, self.greet]
        response_id = self.send_request(
            self.get_request("call_contract_no_changing_state", params), self.__database_api_identifier
        )
...
class PositiveTesting(BaseTest):
    ...
    @lcc.disabled()
    @lcc.test(
        "Check work of 'call_contract_no_changing_state' method, not empty output call contract result "
        "(int and string type)"
    )
    @lcc.depends_on(
        "API.DatabaseApi.Contracts.CallContractNoChangingState.CallContractNoChangingState.method_main_check"
    )
    def check_call_contract_no_changing_state_with_not_empty_call_contract_result(
        self, get_random_integer, get_random_string
    ):
        ...
        check_that(
            "'call contract operation no changing state, output_type=string'",
            result,
            equal_to(contract_output_in_hex),
            quiet=True
        )
        ...
        operation = self.echo_ops.get_contract_call_operation(
            echo=self.echo, registrar=self.echo_acc0, bytecode=self.get_uint, callee=contract_id
        )
        collected_operation = self.collect_operations(operation, self.__database_api_identifier)
        broadcast_result = self.echo_ops.broadcast(
            echo=self.echo, list_operations=collected_operation, log_broadcast=False
        ...
        lcc.set_step("Check call contract operation no changing state equal to call contract result")
        check_that(
            "'call contract operation no changing state, output_type=int'",
            result,
            equal_to(contract_output_in_hex),
            quiet=True
        )
    ...
    @lcc.test(
        "Check work of 'call_contract_no_changing_state' method, empty output call contract result but with "
        "new asset_id"
    )
    @lcc.depends_on(
        "API.DatabaseApi.Contracts.CallContractNoChangingState.CallContractNoChangingState.method_main_check"
    )
    def check_call_contract_no_changing_state_with_empty_call_contract_result(
        self, get_random_valid_asset_name, get_random_integer
    ):
        ...
        self.utils.add_assets_to_account(
            self, value_amount, new_asset_id, self.echo_acc0, self.__database_api_identifier
        )
        ...
        contract_id = self.utils.get_contract_id(
            ...
            supported_asset_id=new_asset_id
        )
