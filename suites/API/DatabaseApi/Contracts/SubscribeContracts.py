...
from lemoncheesecake.matching import (
    check_that, check_that_in, equal_to, has_length, is_, is_integer, is_list, is_none, require_that
)

SUITE = {
    "description":
        "Methods: 'subscribe_contracts', 'get_objects' (contract balance & contract history"
...
@lcc.tags(
    "api", "notice", "database_api", "database_api_contracts", "subscribe_contracts", "database_api_objects",
    "get_objects"
)
...
class SubscribeContracts(BaseTest):
    ...
    def method_main_check(self, get_random_integer):
        ...
        response_id = self.send_request(
            self.get_request("subscribe_contracts", [[contract_id]]), self.__database_api_identifier
        )
...
class PositiveTesting(BaseTest):
    ...
    def check_contract_history_objs_in_notice(response, notice, contract_id):
        ...
        for i, operation_history in enumerate(response):
            ...
            if notice_contract_id != contract_id:
                lcc.log_error(
                    "Notice has incorrect contract id, got '{}', but need '{}'".format(notice_contract_id, contract_id)
                )
            ...
            lcc.log_info(
                "Operations are the same: notice has operation '{}' like contract history='{}'".format(
                    notice_operation_history_id, operation_history_id
                )
            )
            break

    def check_balance_and_statistic_objs_in_notice(
        self, notices, contract_id, contract_balance, asset_type, expected_statistic=True
    ):
        counter = 0
        expected_objs = [
        ...
        for i, notice in enumerate(notices):
            ...
            require_that("'owner'", notice["owner"], equal_to(contract_id), quiet=True)
            ...
            if notice["id"].startswith(expected_objs[0]):
                check_that_in(
                    notice, "asset_type", equal_to(asset_type), "balance", equal_to(contract_balance), quiet=True
                )
    ...
    @lcc.depends_on("API.DatabaseApi.Contracts.SubscribeContracts.SubscribeContracts.method_main_check")
    def check_different_types_of_contract_notices(self, get_random_integer):
        ...
        check_that_in(
            notice, "contract", equal_to(contract_id), "operation_id", equal_to(operation_history_id), quiet=True
        )
        ...
        self.check_balance_and_statistic_objs_in_notice(
            notice_2, contract_id, expected_contract_balance, value_asset_id
        )
        ...
        require_that('results', get_objects_results, equal_to(notice_2), quiet=True)
        ...
        lcc.set_step("Check the identity of returned results of api-methods: 'subscribe_contracts', 'get_objects'")
        require_that('results', get_objects_results, equal_to(notice), quiet=True)
    ...
    def check_notices_of_contract_created_by_another_contract(self, get_random_integer):
        ...
        contract_id = self.utils.get_contract_id(
            self, self.echo_acc0, self.create_contract, self.__database_api_identifier
        )
        ...
        response_id = self.send_request(
            self.get_request("subscribe_contracts", [[created_contract_id]]), self.__database_api_identifier
        )
        self.get_response(response_id)
        ...
        check_that_in(
            notice,
            "contract",
            equal_to(created_contract_id),
            "operation_id",
            equal_to(operation_history_id),
            quiet=True
        )
        ...
        value_asset_id = self.echo_asset
        self.check_balance_and_statistic_objs_in_notice(
            notice, created_contract_id, expected_contract_balance, value_asset_id, expected_statistic=False
        )
