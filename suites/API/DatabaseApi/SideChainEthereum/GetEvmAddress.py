...
SUITE = {
    "description": "Methods: 'get_evm_addresses', 'get_objects' (eth_address object)"
}


@lcc.prop("main", "type")
@lcc.tags(
    "api", "database_api", "sidechain", "sidechain_ethereum", "database_api_sidechain_ethereum", "get_evm_addresses",
    "database_api_objects", "get_objects"
)
@lcc.suite("Check work of methods: 'get_evm_addresses', 'get_objects' (eth address object)", rank=1)
class GetEvmAddress(BaseTest):
    ...
    def evm_address_register_operation(self, get_random_valid_account_name, get_random_eth_address):
        ...
        operation = self.echo_ops.get_evm_address_register_operation(
            echo=self.echo, account=new_account, evm_address=evm_address
        )
        ...
        response_id = self.send_request(
            self.get_request("get_evm_addresses", [new_account]), self.__database_api_identifier
        )
