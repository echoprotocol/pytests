...
@lcc.tags(
    "api", "database_api", "database_api_globals", "get_dynamic_global_properties", "database_api_objects",
    "get_objects"
)
@lcc.suite(
...
class GetDynamicGlobalProperties(BaseTest):
    ...
    @lcc.test("Simple work of methods: 'get_dynamic_global_properties', 'get_objects' (dynamic global property object)")
    def method_main_check(self):
        ...
        require_that('results', get_objects_results[0], equal_to(get_dynamic_global_properties_result), quiet=True)
    ...
    def call_method_with_params(self, get_all_random_types):
        ...
        for i in range(len(get_all_random_types)):
            if i == 4:
                continue
            response_id = self.send_request(
                self.get_request("get_dynamic_global_properties", random_values[i]), self.__api_identifier
            )
