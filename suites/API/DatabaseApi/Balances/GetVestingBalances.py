...
from lemoncheesecake.matching import (
    check_that, check_that_in, equal_to, greater_than, has_length, is_, require_that, require_that_in
)
...
@lcc.tags("api", "database_api", "database_api_balances", "get_vesting_balances", "database_api_objects", "get_objects")
...
class GetVestingBalances(BaseTest):
    ...
    def method_main_check(self, get_random_integer):
        ...
        response_id = self.send_request(
            self.get_request("get_vesting_balances", [self.echo_acc0]), self.__database_api_identifier
        )
        ...
        check_that(
            ...
            get_objects_results,
            equal_to(get_vesting_balances_results),
            quiet=True
        )
...
class PositiveTesting(BaseTest):
    ...
    def in_and_out_vesting_balance_of_new_account(self, get_random_valid_account_name, get_random_integer):
        ...
        operation = self.echo_ops.get_vesting_balance_create_operation(
            echo=self.echo, creator=self.echo_acc0, owner=new_account, amount=value_amount
        )
        ...
        response_id = self.send_request(
            self.get_request("get_vesting_balances", [new_account]), self.__database_api_identifier
        )
        ...
        check_that_in(result, "id", is_(vesting_balance_id), "owner", is_(new_account))
        check_that_in(
            result["balance"], "amount", is_(operation_data["amount"]["amount"]), "asset_id",
            is_(operation_data["amount"]["asset_id"])
        )
        ...
        check_that_in(
            ...
            is_(operation_data["policy"][1]["begin_timestamp"]), "vesting_cliff_seconds",
            is_(operation_data["policy"][1]["vesting_cliff_seconds"]), "vesting_duration_seconds",
            is_(operation_data["policy"][1]["vesting_duration_seconds"]), "begin_balance",
            is_(operation_data["amount"]["amount"])
        )
        ...
        self.utils.perform_vesting_balance_withdraw_operation(
            self, vesting_balance_id, new_account, withdraw_amount_1, self.__database_api_identifier
        )
        ...
        self.utils.perform_vesting_balance_withdraw_operation(
            self, vesting_balance_id, new_account, withdraw_amount_2, self.__database_api_identifier
        )
    ...
    @lcc.depends_on("API.DatabaseApi.Balances.GetVestingBalances.GetVestingBalances.method_main_check")
    def create_vesting_balances_with_several_assets(
        self, get_random_valid_asset_name, get_random_integer, get_random_valid_account_name,
        ...
        self.utils.add_assets_to_account(
            self, new_asset_amount, new_asset, self.echo_acc0, self.__database_api_identifier
        )
        ...
        response_id = self.send_request(
            self.get_request("get_account_balances", [self.echo_acc0, [new_asset]]), self.__database_api_identifier
        )
        response = self.get_response(response_id)["result"][0]
        require_that_in(response, "asset_id", equal_to(new_asset), "amount", equal_to(new_asset_amount))
        ...
        for i, asset in enumerate(assets):
            operation = self.echo_ops.get_vesting_balance_create_operation(
                echo=self.echo, creator=self.echo_acc0, owner=new_account, amount=amounts[i], amount_asset_id=asset
            )
            collected_operation = self.collect_operations(operation, self.__database_api_identifier)
        ...
        for i, vesting_balance in enumerate(vesting_balances):
            ...
            check_that_in(vesting_balance["balance"], "amount", equal_to(amounts[i]), "asset_id", equal_to(assets[i]))

    @lcc.test("Modified begin_timestamp and try to withdraw vesting balance")
    @lcc.depends_on("API.DatabaseApi.Balances.GetVestingBalances.GetVestingBalances.method_main_check")
    def create_vesting_balance_with_begin_timestamp_in_future(
        self, get_random_valid_asset_name, get_random_integer, get_random_valid_account_name
    ):
        ...
        lcc.set_step("Add created assets to account")
        self.utils.add_assets_to_account(self, asset_amount, new_asset, self.echo_acc0, self.__database_api_identifier)
        lcc.log_info("Created '{}' assets added to '{}' account successfully".format(new_asset, self.echo_acc0))
    ...
    def create_vesting_balance_with_vesting_cliff_seconds(
        ...
        vesting_balance_id = self.get_operation_results_ids(broadcast_result)
        lcc.log_info(
            "Vesting balance object '{}' created, vesting_cliff_seconds='{}'".format(
                vesting_balance_id, vesting_cliff_seconds
            )
        )
    ...
    @lcc.depends_on("API.DatabaseApi.Balances.GetVestingBalances.GetVestingBalances.method_main_check")
    def create_vesting_balance_with_vesting_duration_seconds(
        self, get_random_valid_asset_name, get_random_integer, get_random_valid_account_name
    ):
        asset_amount = get_random_integer
        new_asset = get_random_valid_asset_name
        ...
        vesting_balance_id = self.get_operation_results_ids(broadcast_result)
        lcc.log_info(
            "Vesting balance object '{}' created, vesting_duration_seconds='{}'".format(
                vesting_balance_id, duration_seconds
            )
        )
