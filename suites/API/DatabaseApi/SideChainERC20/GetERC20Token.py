...
@lcc.tags(
    "api", "database_api", "sidechain", "sidechain_erc20", "database_api_sidechain_erc20", "get_erc20_token",
...
class GetERC20Token(BaseTest):
    ...
    def method_main_check(self, get_random_string, get_random_valid_asset_name):
        ...
        lcc.log_info(
            "Call method 'get_erc20_token' with eth_erc20_contract_address='{}' parameter".format(
...
class PositiveTesting(BaseTest):
    ...
    def get_info_about_created_erc20_contract(
        self, get_random_string, get_random_valid_asset_name, get_random_integer_up_to_ten
    ):
        contract_name = get_random_string
        erc20_symbol = get_random_valid_asset_name
        ...
        check_that_in(
            ...
            "eth_addr",
            equal_to(erc20_contract.address[2:]),
            "name",
            equal_to(contract_name),
            "symbol",
            equal_to(erc20_symbol),
            "decimals",
            equal_to(erc20_token_decimals),
            quiet=True
        )

    @lcc.test(
        "Create contract using register_erc20_token operation and compare response from 'get_erc20_token' "
        "and 'get_objects'"
    )
    @lcc.depends_on("API.DatabaseApi.SideChainERC20.GetERC20Token.GetERC20Token.method_main_check")
    def compare_with_method_get_objects(self, get_random_string, get_random_valid_asset_name):
        ...
        require_that("result", get_objects_results[0], equal_to(get_erc20_token_result), quiet=True)
