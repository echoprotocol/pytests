...
from lemoncheesecake.matching import (
    check_that, check_that_in, equal_to, greater_than, has_length, is_true, require_that
)
...
@lcc.prop("main", "type")
@lcc.tags(
    "api", "database_api", "sidechain", "sidechain_erc20", "database_api_sidechain_erc20",
    "get_erc20_account_withdrawals", "database_api_objects", "get_objects"
)
@lcc.suite(
    "Check work of methods: 'get_erc20_account_withdrawals', 'get_objects (withdraw erc20 token object)'", rank=1
)
class GetERC20AccountWithdrawals(BaseTest):
    ...
    def check_erc20_account_withdrawals(
        self, withdrawals, account_id, erc20_withdrawal_amounts, withdrawal_erc20_token_ids
    ):
        ...
        for i, erc20_account_withdraw in enumerate(withdrawals):
            ...
            check_that_in(
                ...
                "id",
                equal_to(withdrawal_erc20_token_ids[i]),
                "account",
                equal_to(account_id),
                "is_approved",
                is_true(),
                "value",
                equal_to(str(erc20_withdrawal_amounts[i])),
                quiet=True
            )
    ...
    def method_main_check(self, get_random_valid_account_name, get_random_string, get_random_valid_asset_name):
        ...
        lcc.log_info(
            "Withdrawal ERC20 token completed successfully, withdrawal ERC20 token object is '{}'".format(
        ...
        get_erc20_account_withdrawals_results = self.utils.get_erc20_account_withdrawals(
            self, new_account_id, self.__database_api_identifier
        )["result"]
        self.check_erc20_account_withdrawals(
            get_erc20_account_withdrawals_results, new_account_id, erc20_withdrawal_amounts, withdrawal_erc20_token_ids
        )
        ...
        in_ethereum_erc20_balance_after_first_withdraw = self.eth_trx.get_balance_of(
            erc20_contract, self.eth_account.address
        )
        require_that(
            "'in ethereum erc20 contact balance'",
            str(in_ethereum_erc20_balance_after_first_withdraw),
            equal_to(erc20_withdrawal_amounts[0]),
            quiet=True
        )
        ...
        check_that(
            "'in echo account's erc20 balance'",
            in_echo_erc20_balance_after_second_withdrawal,
            equal_to(in_echo_erc20_balance - int(erc20_withdrawal_amounts[0]) - int(erc20_withdrawal_amounts[1])),
            quiet=True
        )

        lcc.set_step("Get ethereum ERC20 tokens balance after second withdrawal in the Ethereum network")
        in_ethereum_erc20_balance_after_second_withdrawal = self.eth_trx.get_balance_of(
            erc20_contract, self.eth_account.address
        )
        require_that(
            "'in ethereum erc20 contact balance'",
            in_ethereum_erc20_balance_after_second_withdrawal,
            equal_to(in_echo_erc20_start_balance),
            quiet=True
        )
