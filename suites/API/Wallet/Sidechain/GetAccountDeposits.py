...
class GetAccountDeposits(WalletBaseTest, BaseTest):
    ...
    @lcc.test("Simple work of method 'wallet_get_account_deposits'")
    def method_main_check(self, get_random_integer):
        lcc.set_step("Get account deposit")
        result = self.send_wallet_request("get_account_deposits", [self.init5, ""], log_response=True)['result']

        if result == []:
            lcc.log_info("Account has no deposit, add deposit to account")
            ...
            lcc.log_info("Perform sidechain_btc_create_address_operation for account {}".format(self.init5))
            operation = self.echo_ops.get_sidechain_btc_create_address_operation(
                echo=self.echo, account=self.init5, backup_address=backup_address, signer=INIT5_PK
            )
            ...
            lcc.set_step("Get btc address of {} account".format(self.init5))
            ...
            lcc.log_info("'{}' account BTC address is '{}'".format(self.init5, address))

            lcc.set_step("Get {} account btc asset balance".format(self.init5))
            params = [self.init5, ["1.3.2"]]
            response_id = self.send_request(
            ...
            lcc.log_info("'{}' account btc balance = {}".format(self.init5, balance_before_deposit))
        ...
        check_that("account id", result[0]['account'], equal_to(self.init5))
