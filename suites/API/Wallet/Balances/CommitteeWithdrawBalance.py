...
class CommitteeFreezeBalance(WalletBaseTest, BaseTest):
    ...
    def method_main_check(self):
        ...
        withdraw = self.send_wallet_request("committee_withdraw_balance", [self.init4, math.ceil(int(current_frozen_balance_amount) / 2), True], log_response=False)
        lcc.log_info("{}".format(withdraw))
        ...
        lcc.log_info("{}".format(current_frozen_balance_amount))
        lcc.log_info("{}".format(new_frozen_balance_amount))

        check_that("committee frozen balance", int(int(current_frozen_balance_amount) / 2), equal_to(new_frozen_balance_amount))
