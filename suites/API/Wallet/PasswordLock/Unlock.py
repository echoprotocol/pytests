...
SUITE = {
    "description": "Method 'unlock'"
}


@lcc.prop("main", "type")
@lcc.tags("api", "wallet_api", "wallet_password_lock", "wallet_unlock")
@lcc.suite("Check work of method 'unlock'", rank=1)
class Unlock(WalletBaseTest, BaseTest):
    ...
    @lcc.test("Simple work of method 'wallet_unlock'")
    def method_main_check(self):
        ...
        if is_locked:
            ...
            lcc.log_info("Wallet is unlocked, lock to checking the wallet unlock process")
            ...
            if is_locked:
                ...
                self.send_wallet_request("unlock", [WALLET_PASSWORD], log_response=False)['result']
            is_locked = self.send_wallet_request("is_locked", [], log_response=False)['result']
        check_that("wallet lock status", is_locked, equal_to(False))
