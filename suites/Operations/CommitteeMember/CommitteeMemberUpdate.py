...
from fixtures.base_fixtures import get_random_btc_public_key, get_random_eth_address, get_random_url
from project import REQUIRED_DEPOSIT_AMOUNT

import lemoncheesecake.api as lcc
from lemoncheesecake.matching import check_that, check_that_in, equal_to, not_equal_to
...
@lcc.prop("main", "type")
@lcc.tags("operations", "committee_member_operations", "committee_member_update")
@lcc.suite("Check work of operation 'committee_member_update'", rank=1)
class CommitteeMemberUpdate(BaseTest):
    ...
    def method_main_check(self, get_random_valid_account_name):
        ...
        response_id = self.send_request(
            self.get_request("get_committee_member_by_account", [new_account_id]), self.__database_api_identifier
        )
        committee_member = self.get_response(response_id)["result"]
        check_that("'get_committee_member_by_account' method result", committee_member, equal_to(None), quiet=True)

        lcc.set_step("Perform 'committee_member_create' operation")
        operation = self.echo_ops.get_committee_member_create_operation(
            echo=self.echo,
        ...
        self.utils.add_balance_for_operations(
            self, new_account_id, operation, self.__database_api_identifier, deposit_amount=REQUIRED_DEPOSIT_AMOUNT
        )
        collected_operation = self.collect_operations(operation, self.__database_api_identifier)
        broadcast_result = self.echo_ops.broadcast(echo=self.echo, list_operations=collected_operation)
        if not self.is_operation_completed(broadcast_result, expected_static_variant=1):
            raise Exception("Operation 'committee_member_create' failed while broadcast")
        ...
        operation = self.echo_ops.get_committee_member_update_operation(
            echo=self.echo,
        ...
        if not self.is_operation_completed(broadcast_result, expected_static_variant=0):
            raise Exception("Operation 'committee_member_update' failed while broadcast")
        ...
        check_that_in(
            committee_member,
            "id",
            equal_to(committee_member_id),
            "committee_member_account",
            equal_to(committee_member_account),
            "url",
            not_equal_to(old_url),
            "eth_address",
            not_equal_to(old_eth_address),
            "btc_public_key",
            not_equal_to(old_btc_public_key),
            quiet=True
        )
