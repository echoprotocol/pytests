...
from project import INIT4_PK, UTILS
...
class BalanceClaim(BaseTest):
        ...
        if not os.path.exists(UTILS):
            ...
            with open(UTILS, "r") as utils:
                ...
                if "balance_claim_operation" not in utils_data:
                    utils_data.update({"balance_claim_operation": {
                        "state": True
                    }})
                    with open(UTILS, "w") as utils:
                        utils.write(json.dumps(utils_data))
    ...
    def method_main_check(self, get_random_valid_account_name):
        if self.state:
            ...
            lcc.log_info(
                "'{}' account has id='{}' and public_key='{}'".format(self.init4_account_name, account_id, public_key)
            )
            ...
            lcc.log_info(
                "'{}' account has balance with id='{}' and amount='{}'".format(
                    self.init4_account_name, balance_id, balance_amount
                )
            )
            ...
            if execution_status["passed"]:
                ...
                lcc.log_error(
                    "Test of method 'balance_claim_operation' failed during the previous run. "
