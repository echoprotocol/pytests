...
class AccountAddressCreate(BaseTest):
    ...
    def method_main_check(self, get_random_valid_account_name, get_random_string):
        ...
        operation = self.echo_ops.get_account_address_create_operation(echo=self.echo, owner=new_account, label=label)
        ...
        response_id = self.send_request(
            self.get_request("get_account_addresses", [new_account, 0, 1]), self.__database_api_identifier
        )
        ...
        response_id = self.send_request(
            self.get_request("get_account_by_address", [address]), self.__database_api_identifier
        )
