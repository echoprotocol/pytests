...
class OverrideTransfer(BaseTest):
    ...
    def setup_suite(self):
        ...
        lcc.log_info(
            "API identifiers are: database='{}', registration='{}', asset='{}'".format(
    ...
    def method_main_check(self, get_random_valid_asset_name, get_random_integer, get_random_valid_account_name):
        ...
        operation = self.echo_ops.get_asset_create_operation(
            echo=self.echo, issuer=self.echo_acc0, symbol=new_asset, flags=2
        )
        ...
        self.utils.add_assets_to_account(self, new_asset_amount, new_asset, new_account, self.__database_api_identifier)
        ...
        response_id = self.send_request(
            self.get_request("get_asset_holders", [new_asset, _start, _limit]), self.__asset_api_identifier
        )
        ...
        transfer_operation = self.echo_ops.get_override_transfer_operation(
            echo=self.echo,
