...
class AssetClaimFees(BaseTest):
    ...
    def method_main_check(
        self, get_random_valid_account_name, get_random_valid_asset_name, get_random_integer_up_to_ten
    ):
        ...
        asset_issue_operation = self.echo_ops.get_asset_issue_operation(
            echo=self.echo,
        ...
        self.echo_ops.broadcast(echo=self.echo, list_operations=collected_op, log_broadcast=False)
        lcc.log_info("'asset_fund_fee_pool_operation' broadcasted successfully")

        self.utils.perform_transfer_operations(
            self, self.echo_acc0, new_account, self.__database_api_identifier, transfer_amount=20
        )
        operation = self.echo_ops.get_transfer_operation(
            ...
            amount=1,
            fee_amount=20,
            fee_asset_id=new_asset_id,
            amount_asset_id=new_asset_id
        )
        fee = self.get_required_fee(operation, self.__database_api_identifier)["amount"]
        for i in range(transfer_operation_count):
            collected_operation.append(
                self.collect_operations(
                    operation, self.__database_api_identifier, fee_amount=20, fee_asset_id=new_asset_id
                )
            )
        self.echo_ops.broadcast(echo=self.echo, list_operations=collected_operation)
        ...
        response_id = self.send_request(
            self.get_request("get_objects", [[new_asset_id]]), self.__database_api_identifier
        )
        result = self.get_response(response_id)["result"][0]["dynamic_asset_data_id"]
        response_id = self.send_request(self.get_request("get_objects", [[result]]), self.__database_api_identifier)
        ...
        asset_claim_fees_operation = self.echo_ops.get_asset_claim_fees_operation(
            self.echo, issuer=self.echo_acc0, claim_amount=claim_amount, claim_asset_id=new_asset_id
        )
