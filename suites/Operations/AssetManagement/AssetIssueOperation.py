...
SUITE = {
    "description": "Operation 'asset_issue'"
}


@lcc.prop("main", "type")
@lcc.tags("operations", "asset_management_operations", "asset_issue")
@lcc.suite("Check work of method 'asset_issue'", rank=1)
class AssetIssue(BaseTest):
    ...
    @lcc.test("Simple work of method 'asset_issue'")
    def method_main_check(
        ...
        asset_create_operation = self.echo_ops.get_asset_create_operation(
            echo=self.echo, issuer=self.echo_acc0, symbol=new_asset_name
        )
        ...
        lcc.set_step("Check account_balances in new asset")
        response_id = self.send_request(
            self.get_request("get_account_balances", [new_account, [new_asset_id]]), self.__database_api_identifier
        )
