...
class AssetInt(BaseTest):
    ...
    def asset_int_scenario(self):
        ...
        lcc.log_info(
            "'get_account_balances' method return '{}' balance of '{}' account in '{}'".format(
                amount, self.echo_acc0, asset_id
            )
        )
        ...
        operation = self.echo_ops.get_contract_call_operation(
            echo=self.echo, registrar=self.echo_acc0, bytecode=self.asset_balance + method_params, callee=contract_id
        )
        collected_operation = self.collect_operations(operation, self.__database_api_identifier)
        broadcast_result = self.echo_ops.broadcast(echo=self.echo, list_operations=collected_operation)
        contract_result = self.get_contract_result(broadcast_result, self.__database_api_identifier)
        ...
        lcc.log_info(
            "Output: asset balance of '{}' account is '{}' in '{}'".format(
                self.echo_acc0, contract_output, self.echo_asset
            )
        )
