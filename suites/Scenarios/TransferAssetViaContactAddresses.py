...
class TransferAssetViaContactAddresses(BaseTest):
    ...
    def transfer_asset_via_account_address(
        self, get_random_valid_account_name, get_random_string, get_random_integer, get_random_integer_up_to_fifty
    ):
        ...
        lcc.log_info(
            "Account '{}' balance in '{}' asset: '{}'".format(
                new_account, self.echo_asset, balance_before_transfer["amount"]
            )
        )
        ...
        lcc.log_info(
            "Account '{}' balance in '{}' asset: '{}'".format(
                new_account, self.echo_asset, balance_after_transfer["amount"]
            )
        )

        lcc.set_step("Check that transfer assets completed successfully")
        check_that_in(
            balance_after_transfer, "amount", equal_to(balance_before_transfer["amount"] + transfer_amount), "asset_id",
            equal_to(balance_before_transfer["asset_id"])
        )
        ...
        balance_after_second_transfer = self.utils.get_account_balances(
            self, new_account, self.__database_api_identifier
        )
        lcc.log_info(
            "Account '{}' balance in '{}' asset: '{}'".format(
                new_account, self.echo_asset, balance_after_second_transfer["amount"]
            )
        )

        lcc.set_step("Check that second transfer assets completed successfully")
        check_that_in(
            balance_after_second_transfer, "amount", equal_to(balance_after_transfer["amount"] + amount), "asset_id",
            equal_to(balance_after_transfer["asset_id"])
        )
        ...
        check_that_in(
            balance, "amount", equal_to(balance_after_second_transfer["amount"] - withdraw_amount + deposit_amount),
        ...
        lcc.set_step("Check that transfer assets completed successfully")
        check_that_in(
            balance_after_transfer, "amount", equal_to(balance["amount"] + amount), "asset_id",
            equal_to(balance_before_transfer["asset_id"])
        )
