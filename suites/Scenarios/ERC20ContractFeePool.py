...
class ERC20ContractFeePool(BaseTest):
    ...
    def erc20_contract_fee_pool_scenario(self, get_random_string, get_random_valid_asset_name, get_random_integer):
        ...
        response_id = self.send_request(
            self.get_request("get_contract_pool_balance", [erc20_contract_id]), self.__database_api_identifier
        )
        ...
        self.utils.perform_contract_fund_pool_operation(
            self, self.echo_acc0, erc20_contract_id, amount, self.__database_api_identifier
        )
        ...
        check_that(
            "'updated ERC20 contract fee pool balance'", updated_fee_pool_amount_2, equal_to(fee_pool_amount + amount)
        )
