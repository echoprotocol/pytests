...
class HelloWorld(BaseTest):
    ...
    def add_fee_to_account(self, method, contract_id):
        operation = self.echo_ops.get_contract_call_operation(echo=self.echo, registrar=self.account_id,
                                                              bytecode=method, callee=contract_id)
    ...
    def hello_world_scenario(self):
        ...
        self.utils.perform_transfer_operations(self, self.echo_acc0, self.account_id, self.__database_api_identifier,
                                               transfer_amount=self.value_amount)

        lcc.set_step("Create 'Piggy' contract in the Echo network")
        operation = self.echo_ops.get_contract_create_operation(echo=self.echo, registrar=self.account_id,
