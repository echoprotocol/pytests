...
from lemoncheesecake.matching import require_that, equal_to, check_that, is_false, has_length, is_list, is_dict, \
    check_that_in, is_integer, is_str
...
class HistoryOfContractCreatedByAnotherContract(BaseTest):
    ...
    def check_contract_internal_create_op(self, contract_id, stop, limit, start, caller, new_contract):
        ...
        if require_that("'contract_internal_create_operation keys'", op_keys,
                        has_length(5), quiet=True):
            check_that("'contract_internal_create_operation id'", operation_id,
                       equal_to(self.contract_internal_create_operation_id), quiet=True)
    ...
    def check_contract_internal_call_op(self, contract_id, stop, limit, start, caller, new_contract):
        ...
        operation_id, operation_dict = operation[0], operation[1]
        op_keys = [key for key in operation_dict.keys()]
        if require_that("'contract_internal_call_operation keys'", op_keys,
                        has_length(5), quiet=True):
            check_that("'contract_internal_call_operation id'", operation_id,
                       equal_to(self.contract_internal_call_operation_id), quiet=True)
            check_that("'caller id'", caller, equal_to(operation_dict["caller"]))
    ...
    def history_of_contract_created_by_another_contract(self):
        ...
        self.check_contract_internal_create_op(created_contract_id, stop, limit, start, contract_id,
                                               created_contract_id)
        ...
        self.check_contract_internal_call_op(created_contract_id, stop, limit, start, created_contract_id,
                                             contract_id)

        lcc.set_step("Check 'contract_internal_call_operation' in contract history")
        self.check_contract_internal_call_op(contract_id, stop, limit, start, created_contract_id,
                                             contract_id)
